...
class graphManager{ // all funcs must be called from
    ...
    let graphNameArray = ["RPM", "Torque (N•m)", "Throttle (%)", "Duty (%)", "PWM Frequency", "Temperature (C)", "Source Voltage(V)", "PWM Current (A)", "Power Change (ΔW)", "Voltage Change (ΔV)"];
    ...
    public func updateGraph(with index: Int, point: ChartDataEntry) -> Bool{
        ...
        if (result){
            ...
            while (graphViews[index].data!.dataSets[0].entryCount > bufferSize){ // replace 100 with custom size
